...
const handleUpdateProduction = async (productionId) => {
  try {
    ...
    setEditValue({ quantity: '', workers: '', displayQuantity: '', displayTarget: '' });
    fetchProjectDetails();
  } catch (error) {
                        ...
                        {/* {editingProduction === item.id && canEdit ? (
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <input
                                    type="number"
                                    value={editValue.displayQuantity}
                                    onChange={(e) => setEditValue({...editValue, displayQuantity: e.target.value})}
                                    placeholder={item.display_quantity || item.quantity_produced}
                                    className="flex-1 px-2 py-1 bg-slate-700 border border-green-600 rounded text-white text-sm"
                                    />
                                </div>
                                <div className="flex items-center gap-2">
                                    <label className="text-slate-400 text-xs w-20">Target:</label>
                                    <input
                                    type="number"
                                    value={editValue.displayTarget}
                                    onChange={(e) => setEditValue({...editValue, displayTarget: e.target.value})}
                                    placeholder={item.display_target || item.target_quantity}
                                    className="flex-1 px-2 py-1 bg-slate-700 border border-green-600 rounded text-white text-sm"
                                    />
                                </div>
                                <p className="text-xs text-slate-500 italic">Leave empty to use actual values</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    <span className="text-green-300 text-sm">
                                        Client sees: {item.display_quantity || item.quantity_produced} / {item.display_target || item.target_quantity} {item.product_unit}
                                    </span>
                                    )}
                                </div>
            ...
            {/* <div className="bg-slate-800 bg-opacity-50 backdrop-blur-sm border border-slate-700 rounded-xl p-6">
              ...
              <div className="grid grid-cols-3 gap-4 mb-4 p-3 bg-slate-900 bg-opacity-50 rounded-lg">
                <div className="text-center">
                  ...
                  <p className="text-sm font-bold text-green-400">{formatCurrency(payments.totals.advance_paid || 0)}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-slate-400">Partial</p>
                  <p className="text-sm font-bold text-blue-400">{formatCurrency(payments.totals.partial_paid || 0)}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-slate-400">Final</p>
                  <p className="text-sm font-bold text-purple-400">{formatCurrency(payments.totals.final_paid || 0)}</p>
                </div>
              </div>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {payments.payments.map((payment) => (
                    ...
                    >
                      <div>
                        <p className="text-white font-medium">{formatCurrency(payment.amount)}</p>
                        <p className="text-xs text-slate-400 capitalize">{payment.payment_type} - {payment.payment_method}</p>
